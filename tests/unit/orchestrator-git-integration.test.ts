...
const TASK_AGENT = `const fs = require("node:fs");
...
function createRepo(workspaceMode: "shared" | "git_worktree"): string {
  ...
  writeFileSync(join(repoRoot, "fake-agent.cjs"), TASK_AGENT);
  writeFileSync(
...
describe("git integration dirty checkout guard", () => {
  it("refuses git-worktree integration when the shared checkout is dirty", () => {
    ...
    expect(errors[0]).toContain("TASK-001 cannot cherry-pick into main");
